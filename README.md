...
- **Execution state (SKILL.state, arXiv 2608.26263).** Per-run bounded structured state: agents propose JSON patches (`null` deletes a key); the runtime validates, merges (`Σ_{t+1} = Σ_t ⊕ ΔΣ_t`), and versions. Malformed or stale-version patches change nothing. Reasoning traces are never stored. A board lane's ▶ seeds a run from its cards in canvas order (`run-lane`); the lane header shows the run's step counter and opens the station on it. The station stays a plugin the owner enables per workspace.
